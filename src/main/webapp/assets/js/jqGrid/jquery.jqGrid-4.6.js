...
//alert(JSON.stringify(sdata));
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function postIt() {
				//alert($.jgrid.ajaxOptions);
				//alert(rp_ge[$t.p.id].ajaxEditOptions);
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						//alert( $.isFunction(rp_ge[$t.p.id].serializeDelData) ? rp_ge[$t.p.id].serializeDelData.call($t,postd) : postd);
						var postid = $.isFunction(rp_ge[$t.p.id].serializeDelData) ? rp_ge[$t.p.id].serializeDelData.call($t,postd) : postd;
						//alert($.parseJSON(postid).id);
						var ajaxOptions = $.extend({
							//url: rp_ge[$t.p.id].url || $($t).jqGrid('getGridParam','editurl'),
							url: rp_ge[$t.p.id].url || ($($t).jqGrid('getGridParam','delurl')+'?id='+$.parseJSON(postid).id),
							type: rp_ge[$t.p.id].mtype,
							contentType:'application/json;charset=UTF-8',

...
<script>
		$(document).ready(function(){
			  $('#maincodegrid').datagrid({  
				    url:'${ctx}/jsonbnufindallcodetype',  
				    rownumbers:true,//
					singleSelect:true,
					toolbar:'#tb1',
					fit:true,
			    ...
			    columns:[[  
			        {field:'typeValue',title:'代码类别',width:80},
					{field:'typeName',title:'代码名称',width:200}
			    ]],
...
<script>		
		...
		function addCodeType(){  				
		    ...
		    $('#fm').form('clear');   
		    urlCodeType = '${ctx}/jsonbnusavecodetype'; 
		} 
		    ...
		    //alert(row.id);
				if (row){
				    $.ajax({
							type:'POST',
							async:false,
							url:'${ctx}/jsonbnuremovecodetypebyid?id='+row.id,
							dataType: 'json',
		...
		function editCodeType(){  
		    ...
		    //alert(row);
				if (row){  
				    ...
				    $('#fm').form('load',row);  
				    urlCodeType = '${ctx}/jsonbnusavecodetype?id='+row.id;  
...
<script>
		function queryCodeByType(typeValue){
			$('#detailgrid').datagrid({  
			    url:'${ctx}/jsonbnufindallcodebytype?codeType='+typeValue,  
					toolbar:'#tb',
			    rownumbers:true,// 
					singleSelect:true,
			    columns:[[  
		    ...
		    alert(row.id);
				if (row){
				    $.ajax({
							type:'POST',
							async:false,
							url:'${ctx}/jsonbnuremovecodebyid?id='+row.id,
							dataType: 'json',
							contentType : 'application/json',
							success: function(result){  
								alert(result.success);
		...
		function editCode(){
		    ...
		    //alert(row);
				if (row){  
				    ...
				    $('#fm1').form('load',row);  
				    urlCode = '${ctx}/jsonbnusavecode?id='+row.id;  
		...
		<div id="maincode" data-options="region:'west',title:'代码类别',split:true" style="width:500px;">
			<div id="maincodegrid">
    ...
    <div id="detailcode" data-options="region:'center',title:'代码'" style="padding:1px;background:#eee;">
    		<div id="detailgrid">
			    ...
			    <div style="margin-bottom:5px">  
			        <a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onClick="addCode()"></a>  
